    ...
    with open(csv_file, "r", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            try:
                ...
                params = {"movieId": movie_id, "person_tmdbId": person_id, "role": role}
    ...
    with open(csv_file, "r", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            try:
                ...
                params = {"movieId": movie_id, "person_tmdbId": person_id}
...
def verify_import():
    ...
    person_count = run_read_query("MATCH (p:Person) RETURN count(p) as count")[0][
        "count"
    ]
    acted_in_count = run_read_query(
        "MATCH (p:Person)-[r:ACTED_IN]->(m:Movie) RETURN count(r) as count"
    )[0]["count"]
    directed_count = run_read_query(
        "MATCH (p:Person)-[r:DIRECTED]->(m:Movie) RETURN count(r) as count"
    )[0]["count"]
